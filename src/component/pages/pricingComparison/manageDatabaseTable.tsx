...
const ManageDatabaseTable = () => {
  ...
  // Filter function
  const filterData = (managedDatabaseData: PricingData[]) => {
    return managedDatabaseData.filter((item) => {
      // Provider filter
  ...
  const filteredManagedDatabaseData = useMemo(() => filterData(managedDatabaseData), [filters]);
  const clearFilters = () => {
    setFilters({
      provider: "",
      vcpuMin: "",
  ...
  return (
    <div className="bg-white rounded-[1rem] p-[1rem]">
      <div className="flex items-center justify-between gap-[5rem] text-[black] pl-[1rem] mb-[1rem]">
        <div>
          <p className="text-[1.5rem] font-semibold">Manage Databases</p>
        </div>
        <div className=" border border-[#d7e6fa] bg-[#d7e6fa] rounded-[1rem] px-[1rem] py-[0.2rem] text-[13px] font-semibold">
          <p className="flex gap-[5px] text-[#1e40b0]">
            <TipsAndUpdatesIcon className="!w-[15px] !h-[20px]" /> Al Best Fit:
            DigitalOcean (Easiest to manage with clear pricing.)
          </p>
        </div>
      </div>
      ...
      <div className="col-span-10">
        <div className="bg-white rounded-[1rem] p-[1rem]">
          {/* <h2 className="text-black text-[1.5rem] font-semibold mb-4">Manage Databases Filters</h2> */}
      ...
      <Table className="mb-[1.5rem]">
        ...
        <TableBody>
          {filteredManagedDatabaseData.map((row) => (
...
export default ManageDatabaseTable;

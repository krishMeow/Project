import {
  ...
  import { kubernetesData, type PricingData } from "./form.state";
...
import { regions } from "../../../utils/helperData";
  ...
  const KubernetesTable = () => {
    ...
    // Filter function
    const filterData = (kubernetesData: PricingData[]) => {
      return kubernetesData.filter((item) => {
        // Provider filter
    ...
    const filteredKubernetesData = useMemo(() => filterData(kubernetesData), [filters]);
    const clearFilters = () => {
      setFilters({
        provider: "",
        vcpuMin: "",
    ...
    return (
      <div className="bg-white rounded-[1rem] p-[1rem]">
        <div className="flex items-center justify-between gap-[5rem] text-[black] pl-[1rem] mb-[1rem]">
          <div>
            <p className="text-[1.5rem] font-semibold">Kubernetes Clusters</p>
          </div>
          <div className=" border border-[#d7e6fa] bg-[#d7e6fa] rounded-[1rem] px-[1rem] py-[0.2rem] text-[13px] font-semibold">
            <p className="flex gap-[5px] text-[#1e40b0]">
              <TipsAndUpdatesIcon className="!w-[15px] !h-[20px]" /> Al Best Fit: Google Cloud (GKE offers the most mature feature set and
auto-scaling.)
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[1rem] p-[1rem]">
          {/* <h2 className="text-black text-[1.5rem] font-semibold mb-4">Kubernetes Clusters Filters</h2> */}
        ...
        <TableHead>
          ...
          <TableBody>
            {filteredKubernetesData.map((row) => (
